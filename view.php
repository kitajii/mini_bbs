...
if (empty($_REQUEST['id'])) { //URLのid=が空だったら。（日付ボタンのaタグで'id'と指定されている為、$_REQUEST['id']となる。）
  header('Location: index.php');  //空だったら、index.phpに戻る。
  exit();
}
...
<?php if($post = $posts->fetch()): ?> <!-- fetchされた値が$postに入ればtrue つまりここからエラーじゃない時の動作を定義する。 -->
      <div class="msg">
